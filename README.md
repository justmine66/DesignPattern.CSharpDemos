...
抽象工厂是对工厂方法的进一步抽象，创建提供相关依赖对象的抽象工厂，而不需指定它们具体的类。比如我们有一个DataAccess对象里面定义CreateFactory工厂的方法，根本不同的参数创建不同的抽象工厂来兼容不同的数据库驱动。所以**工厂方法和抽线工厂最大的不同就是工厂方法创建出来的是实际的对象，而抽象工厂创建出来的是工厂**。
